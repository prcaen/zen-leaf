...
export default function SelectRoomScreen() {
  ...
  const { rooms, plants, getPlantById, updatePlant } = usePlants();
  const [filterIndoor, setFilterIndoor] = useState(true);

  const plant = getPlantById(plantId);
  const plantLightNeeded = plant?.careInfo?.lightNeeded;
  ...
  const handleCreateRoom = () => {
    router.push(`/room/create?returnTo=/plant/create/${plantId}/room`);
  };
